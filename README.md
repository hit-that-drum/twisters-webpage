...
사용자는 회원가입/로그인을 통해 사이트의 내부에 접근할 수 있으며, 로컬 계정은 이메일 인증 후 관리자 승인을 받아야 접근 가능합니다.<br>
...
Users can access the internal site through signup/login, and local accounts require email verification plus administrator approval before access is granted.<br>
...
- Email verification flow for local signup
- Admin approval flow for final account activation
- Google OAuth login
- Kakao OAuth login
- Role-based access control for admin/member permissions

### Local account activation flow

1. A user signs up with name/email/password.
2. The backend sends a signup verification email.
3. The user opens the verification link, which lands on `/signin` and consumes the verification token.
4. The account remains blocked until an administrator approves it.
5. After both steps are complete, local signin succeeds.

Google and Kakao accounts skip the email-verification email step because the provider login is treated as verified, but they still require administrator approval before access.
